...
/**
 * Class to draw the flag-game
 */
public class GeographyFlagView extends JPanel implements KeyListener, ActionListener{
	
	private ViewableFlagGame game;
	...
	public GeographyFlagView(ViewableFlagGame game) {
		this.game = game;
	...
	protected void paintComponent(Graphics g) {
		...
		if(!game.gameOver()) {
		    ...
		    for(int i = 0; i < 4 ; i++) {
		    	...
		    	String flagPath = game.getFlagAlternatives().get(i);
		    	BufferedImage image = Inf101Graphics.loadImageFromResources(flagPath);	
		    	Inf101Graphics.drawCenteredImage(g2, image, rec.getCenterX(), rec.getCenterY(), 0.20);
		    	
		    	if(flagPath.equals(game.getRandomCounry().getFlagPath())) {
			...
			public void mouseReleased(MouseEvent e) {
				...
				if(mouseIsInTheRectangle && !game.gameOver()) {
					if(mouseIsInTheRectangle = correctGuessRectangle.contains(e.getPoint())) {
						game.correctGuess(game.getRandomCounry().getFlagPath());
